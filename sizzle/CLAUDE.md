...
- **CC-400 inciting incidents** — three of four are implemented and pending first-play review: `the Toronto Blackout` (`blackout.twee`), `the Dark of Manitoulin` (`manitoulin.twee`), and `the Woman with the Pale Eyes` (`pale.twee`). `Wet Dog Smell` is scaffolded and wired in CC-400 (`wds.twee`) but its drafting placeholders are outstanding. See `docs/INCIDENTPLAN.md` and `docs/incidents/`.
